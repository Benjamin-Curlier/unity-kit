...
## Going multiplayer later? (Netcode for Entities)
The sim/presentation split, pure rules, and event counters carry straight into netcode — but three of this skill's defaults flip: replicated entities must be **ghost prefabs** (SubScene-baked or runtime `GhostPrefabCreation.ConvertToGhostPrefab`) — live `CreateEntity` instances don't replicate, and only the server spawns instances; the **input singleton** becomes per-player `IInputComponentData` on the owned ghost; and a **`DeltaTime` accumulator** doesn't survive prediction rollback — schedule by absolute tick. If multiplayer is even plausibly on the roadmap, read **unity-netcode-entities** before choosing this skill's shortcuts.
